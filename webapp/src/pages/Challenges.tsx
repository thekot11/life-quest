...
export function Challenges() {
  ...
  function handleGenerate() {
    generateChallenge(selectedCat || undefined);
    loadChallenges();
    ...
    <div className="p-4 pb-20">
      ...
      <div className="flex gap-2 overflow-x-auto pb-3 mb-4 -mx-1 px-1">
        <button
          ...
          className={`shrink-0 px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${
            selectedCat === null ? 'bg-primary-500 text-white' : 'bg-gray-100 text-gray-600'
          }`}
        >
          🎯 Все
        </button>
        {CATEGORIES.map((cat) => (
          <button
            ...
            className={`shrink-0 px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${
              selectedCat === cat.id ? 'bg-primary-500 text-white' : 'bg-gray-100 text-gray-600'
            }`}
          >
            {cat.emoji} {cat.name}
          </button>
      ...
      <div className="flex gap-2 mb-4">
        <button
          ...
          className="flex-1 bg-gradient-to-r from-primary-500 to-indigo-500 text-white font-medium py-3 rounded-xl active:scale-95 transition-transform"
        >
          🔀 Получить челлендж
        </button>
        <button
